defmodule Barragenspt.Workers.BuildPrecipitationWmsCache do
  ...
  def spawn_workers do
    combinations =
      for year <- 1981..2023,
          month <- 1..12,
          layer <- ["mrrto.obsSup.daily.vector.conc", "mrrto.obsSup.monthly.vector.baciasHidro"],
  ...
  defp build_worker(year, month, layer, img_format) do
    Barragenspt.Workers.BuildPrecipitationWmsCache.new(%{
      "year" => year,
      "month" => month,
      "format" => img_format,
      "layer" => layer
  ...
  def perform(%Oban.Job{
    ...
    {:ok, _ret} = Barragenspt.Services.BarragensPtClient.get_wms_precipitation(query_params)
    :ok
  end
end
