defmodule Barragenspt.Hydrometrics.StorageReportAi do
  @moduledoc """
  Builds an AI-ready summary from the existing storage report output.
  """
  ...
  defp system_prompt do
    ...
    Objetivo:
    - resumir o estado atual do armazenamento
    - destacar principais sinais positivos e negativos
  ...
  defp user_prompt(facts) do
    """
    Abaixo segue o output estruturado de um relatório semanal de armazenamento.
  ...
  defp facts_blob(report) do
    ...
    current =
      [
        ...
        "Data de referência: #{format_datetime(report.report_date)}",
        ...
        "Variação semanal: #{format_delta(summary.week_delta)}",
    ...
    basin_lines =
      basins
      |> Enum.map(fn basin ->
        "- #{basin.name}: atual #{format_pct(basin.current_pct)}, semana #{format_delta(basin.week_delta)}, referência #{format_delta(basin.reference_delta)}, volume #{format_volume(basin.current_volume)}, estado #{status_label(basin.status)}, barragens #{basin.dam_count}"
      end)
      |> Enum.join("\n")
    ...
    strongest_positive =
      basins
      |> Enum.filter(&is_number(&1.week_delta))
      |> Enum.sort_by(& &1.week_delta, :desc)
      |> Enum.take(3)
      |> Enum.map(fn basin -> "- #{basin.name}: #{format_delta(basin.week_delta)}" end)
      |> empty_fallback("- Sem dados suficientes.")

    strongest_negative =
      basins
      |> Enum.filter(&is_number(&1.week_delta))
      |> Enum.sort_by(& &1.week_delta, :asc)
      |> Enum.take(3)
      |> Enum.map(fn basin -> "- #{basin.name}: #{format_delta(basin.week_delta)}" end)
      |> empty_fallback("- Sem dados suficientes.")
    ...
    [
      ...
      dam_lines,
      "## Maiores subidas semanais",
      strongest_positive,
      "## Maiores descidas semanais",
      strongest_negative
    ]
    |> Enum.join("\n\n")
  end
