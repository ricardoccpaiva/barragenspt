defmodule BarragensptWeb.DataPointsFilterSingleSelect do
  ...
  def data_points_filter_single_select(assigns) do
    ...
    <div
      ...
      phx-click-away={if(@open?, do: "data_points_single_select_close")}
        ...
        <%= for {label, opt_val} <- @options do %>
          ...
          <li role="none">
            <button
              ...
              class={[
                "flex w-full items-center gap-2 px-3 py-2 text-left text-[13px] text-slate-800 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-700/80",
                selected? &&
