                  ...
                  <%= if Map.get(item, :children, []) != [] do %>
                    ...
                    >
                      <button
                        ...
                        aria-haspopup="menu"
                        class="inline-flex h-8 cursor-pointer items-center gap-1 rounded-lg px-2.5 text-sm font-semibold leading-none text-slate-500 hover:bg-slate-100/90 focus:bg-slate-100/90 focus:outline-none dark:text-slate-400 dark:hover:bg-slate-700/60 dark:focus:bg-slate-700/60"
                      >
                        <.icon name={item.icon} class="h-3.5 w-3.5 shrink-0 opacity-80" />
                        <span>{item.label}</span>
                      </button>

                      <div class="invisible pointer-events-none absolute left-0 top-full z-50 w-40 pt-1 opacity-0 transition-all duration-150 group-hover:visible group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:visible group-focus-within:pointer-events-auto group-focus-within:opacity-100">
